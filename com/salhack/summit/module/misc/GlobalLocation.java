            ...
            if (event.getStage() == MinecraftEvent.Stage.Pre) {
                if (event.getPacket() instanceof SPacketSpawnMob) {
                    ...
                    if (this.thunder.getValue() && packet2.getCategory() == SoundCategory.WEATHER && packet2.getSound() == SoundEvents.ENTITY_LIGHTNING_THUNDER) {
                        ...
                        Summit.SendMessage("Lightning spawned X:" + Minecraft.getMinecraft().player.posX + " Z:" + Minecraft.getMinecraft().player.posZ + " Angle:" + yaw2);
                    }
                }
                    ...
                    if (packet3.getSoundType() == 1023 && this.Wither.getValue()) {
                        ...
                        angle2 = angle - 180.0;
                        Summit.SendMessage("Wither spawned in direction " + angle2 + " with y position: " + packet3.getSoundPos().getY());
                    }
                    else if (packet3.getSoundType() == 1038 && this.EndPortal.getValue()) {
                        Summit.SendMessage("End portal spawned at " + packet3.getSoundPos().toString());
                    }
                    else if (packet3.getSoundType() == 1028 && this.EnderDragon.getValue()) {
                        Summit.SendMessage("Ender dragon died at " + packet3.getSoundPos().toString());
                    }
                }
            }
        });
