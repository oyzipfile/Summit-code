...
public class Notifications extends Module
{
    ...
    public Notifications() {
        ...
        this.onPlayerUpdate = new Listener<EventPlayerUpdate>(event -> {
            if (Minecraft.getMinecraft().inGameHasFocus) {
                ...
                if (SummitStatic.AUTOWALK.isEnabled() && this.Stuck.getValue() && Math.abs(this.mc.player.motionX) < 0.1 && Math.abs(this.mc.player.motionZ) < 0.1) {
    ...
    public void init() {
        ...
        final String path = this.getClass().getResource("/assets/salhack/imgs/summitlogo.png").getPath();
        this.image = Toolkit.getDefaultToolkit().createImage(path);
        (this.trayIcon = new TrayIcon(this.image, "Summit")).setImageAutoSize(true);
        this.trayIcon.setToolTip("Summit Notifications");
    }
    ...
    public void onEnable() {
        ...
        this.sendNotificationIfNeed("Initalized Summit Notifications Module!");
    }
    ...
    private void sendNotificationIfNeed(final String msg) {
        try {
            this.trayIcon.displayMessage("Summit", msg, TrayIcon.MessageType.INFO);
        }
        catch (Exception ex) {}
    }
}
