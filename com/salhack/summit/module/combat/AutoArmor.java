            ...
            if (!(this.mc.currentScreen instanceof GuiInventory)) {
                if (!SummitStatic.AUTOMEND.isEnabled()) {
                    ...
                    if (this.ElytraReplace.getValue() && !this.mc.player.inventoryContainer.getSlot(6).getStack().isEmpty()) {
                        ...
                        if (ArmorComponent.GetPctFromStack(stack) < 3.0f && SummitStatic.AUTOWALK.isEnabled() && SummitStatic.STASHFINDER.automaticWalk.getValue()) {
                            this.SendMessage("Turned Off Autowalk");
                            SummitStatic.AUTOWALK.toggle();
                        }
                        if (ArmorComponent.GetPctFromStack(stack) > 10.0f && !SummitStatic.AUTOWALK.isEnabled() && SummitStatic.STASHFINDER.automaticWalk.getValue() && this.mc.player.ticksExisted % 25 == 0) {
                            SummitStatic.AUTOWALK.toggle();
